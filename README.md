the_titanium_searcher
=====================

| **linux**                                   | **darwin**                                  | **windows**                                   |
|:-------------------------------------------:|:-------------------------------------------:|:---------------------------------------------:|
| [![travis-ci.org][travis-badge]][travis-ci] | [![circleci.com][circleci-badge]][circleci] | [![appveyor-badge][appveyor-badge]][appveyor] |
| **godoc.org**                               | **codecov.io**                              |
| [![godoc.org][godoc-badge]][godoc] | [![codecov.io][codecov-badge]][codecov] |

A code search tool using *SIMD* instructions(*SSE*, *AVX*) written in Go.

First goal is fully compatible and faster than [the_platinum_searcher (`pt`)][pt].

[![Analytics][ga-badge]][ga]

Benchmark target
----------------

- [GNU grep][gnu-grep]
- [git grep][git-grep]
- [petdance/ack2][ack2]
- [ggreer/the_silver_searcher (`ag`)][ag]
- [monochromegane/the_platinum_searcher (`pt`)][pt]
- [BurntSushi/ripgrep (`rg`)][rg]
- [svent/sift][sift]

TODO
----
- [ ] Use SIMD instructions
- [ ] Support multi encodings
  - [ ] `IOS-2022-JP`
  - [ ] `EUC-JP`
  - [ ] `Shift_JIS`



[gnu-grep]: https://www.gnu.org/software/grep/
[git-grep]: https://git-scm.com/docs/git-grep
[ack2]: https://github.com/petdance/ack2
[ag]: https://github.com/ggreer/the_silver_searcher
[pt]: https://github.com/monochromegane/the_platinum_searcher
[rg]: https://github.com/BurntSushi/ripgrep
[sift]: https://github.com/svent/sift

[travis-ci]: https://travis-ci.org/zchee/the_titanium_searcher
[circleci]: https://circleci.com/gh/zchee/the_titanium_searcher
[appveyor]: https://ci.appveyor.com/project/zchee/the_titanium_searcher
[godoc]: https://godoc.org/github.com/zchee/the_titanium_searcher
[codecov]: https://codecov.io/gh/zchee/the_titanium_searcher
[ga]: https://github.com/zchee/the_titanium_searcher

[travis-badge]: https://img.shields.io/travis/zchee/the_titanium_searcher.svg?style=flat-square&label=%20Travis%20CI&logo=data%3Aimage%2Fsvg%2Bxml%3Bcharset%3Dutf-8%3Bbase64%
[circleci-badge]: https://img.shields.io/circleci/project/github/zchee/the_titanium_searcher/master.svg?style=flat-square&label=%20%20CircleCI&logoWidth=16&logo=data%3Aimage%2Fsvg%2Bxml%3Bcharset%3Dutf-8%3Bbase64%%3D%3D
[appveyor-badge]: https://img.shields.io/appveyor/ci/zchee/the_titanium_searcher/master.svg?style=flat-square&label=AppVeyor&logo=data%3Aimage%2Fsvg%2Bxml%3Bcharset%3Dutf-8%3Bbase64%%3D%3D
[godoc-badge]: https://img.shields.io/badge/godoc-reference-4F73B3.svg?style=flat-square&label=%20godoc.org
[codecov-badge]: https://img.shields.io/codecov/c/github/zchee/the_titanium_searcher.svg?style=flat-square&label=%20%20Codecov%2Eio&logo=data%3Aimage%2Fsvg%2Bxml%3Bcharset%3Dutf-8%3Bbase64%
[ga-badge]: https://ga-beacon.appspot.com/UA-89201129-1/zchee/the_titanium_searcher?flat&useReferer&pixel
